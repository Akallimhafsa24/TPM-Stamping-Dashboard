...
def load_data(f) -> pd.DataFrame:
    """
    Read a Streamlit UploadedFile (or a file path string) into a DataFrame.
    Bytes are read only once and stored in session_state to survive reruns.
    """
    # Accept a plain file path (string) for library loads
    if isinstance(f, str):
        try:
    ...
    # Streamlit UploadedFile — read bytes once and cache to avoid double-read
    name = f.name.lower()
    cache_key = f"_raw_bytes_{f.name}_{f.size}"
    ...
    try:
        ...
        if df.empty or len(df.columns) == 0:
            st.error("File parsed but no columns found. Check format/separator.")
            return pd.DataFrame()
        return df
    except Exception as e:
        st.error(f"File read error: `{e}`")
...
with st.sidebar:
    ...
    if _archive_files:
        ...
        _sel_session = st.selectbox(
            ...
            options=["— choose a session —"] + _arch_labels,
            key="lib_selectbox",
            label_visibility="visible"
        )
        if st.button("▶ Load Selected Session", key="btn_load_library",
                     use_container_width=True,
                     disabled=(_sel_session == "— choose a session —")):
...
if _src == "upload":
    ...
    if _is_new_file:
        # Read once and cache in session_state (load_data handles caching)
        _df_new = load_data(uploaded)
        if _df_new.empty:
            st.stop()
        # Archive immediately on first read
        _arc_name = archive_import(_df_new)
        if _arc_name:
            st.session_state["_archive_saved"] = _arc_name
        # Save to persistent so X-click keeps data alive
        ...
        st.session_state.edited_df     = None
    df_raw = load_data(uploaded)    # safe: uses cached bytes
    ...
    # ── Priority 3: Persistent CSV (auto-load on refresh) ─────────
